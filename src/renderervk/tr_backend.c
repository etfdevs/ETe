...
static const void* RB_Draw2dPolys( const void* data ) {
	...
	for ( i = 0; i < cmd->numverts; i++ ) {
		...
		tess.vertexColors[ tess.numVertexes + 0].u32 =
		tess.vertexColors[ tess.numVertexes + 1].u32 =
		tess.vertexColors[ tess.numVertexes + 2].u32 =
		tess.vertexColors[ tess.numVertexes + 3].u32 = cmd->verts[i].modulate.u32;
		tess.numVertexes++;
	}
...
void RB_ShowImages( void )
{
	...
	for ( i = 0; i < tr.numImages; i++ ) {
		...
		float w = glConfig.vidWidth / 40;
		float h = glConfig.vidHeight / 30;
		float x = i % 40 * w;
		float y = i / 30 * h;
