...
//----(SA) end
		...
		else if ( !Q_stricmp( token, "alphaGen" ) )
		{
			...
			{
				stage->bundle[0].alphaGen = AGEN_NORMALZFADE;
				token = COM_ParseExt( text, qfalse );
				if ( token[0] )
				{
					stage->bundle[0].constantColor.rgba[3] = 255 * Q_atof( token );
				...
				{
					stage->bundle[0].constantColor.rgba[3] = 255;
				}

				token = COM_ParseExt( text, qfalse );
				if ( token[0] )
				{
					stage->bundle[0].zFadeBounds[0] = Q_atof( token );    // lower range
					token = COM_ParseExt( text, qfalse );
					stage->bundle[0].zFadeBounds[1] = Q_atof( token );    // upper range
				}
				else
				{
					stage->bundle[0].zFadeBounds[0] = -1.0;   // lower range
					stage->bundle[0].zFadeBounds[1] =  1.0;   // upper range
				}
