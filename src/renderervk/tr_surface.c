...
static void DoRailCore( const vec3_t start, const vec3_t end, const vec3_t up, float len, float spanWidth )
{
	...
	tess.vertexColors[tess.numVertexes].u32 = backEnd.currentEntity->e.shader.u32;
	tess.numVertexes++;
...
static void RB_SurfacePolyBuffer( srfPolyBuffer_t *surf ) {
	...
	for ( i = 0; i < surf->pPolyBuffer->numVerts; i++ ) {
		...
		tess.vertexColors[numv].u32 = surf->pPolyBuffer->color->u32;
		numv++;
	}
...
void RB_SurfaceDecal( srfDecal_t *srf ) {
	...
	for ( i = 0; i < srf->numVerts; i++ )
	{
		...
		tess.vertexColors[ numv ].u32 = srf->verts[ i ].modulate.u32;
		numv++;
	}
