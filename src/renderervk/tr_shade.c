...
static void ProjectDlightTexture( void ) {
	...
	float intensity, remainder, modulate = 0.0f;
...
void R_ComputeColors( const int b, color4ub_t *dest, const shaderStage_t *pStage )
{
	...
	case AGEN_NORMALZFADE:
		RB_CalcNormalZFade( pStage->bundle[b].constantColor.rgba[3], pStage->bundle[b].zFadeBounds, dest->rgba );
		break;
		// done.
	case AGEN_VERTEX:
		for ( i = 0; i < tess.numVertexes; i++ ) {
...
uint32_t VK_PushUniform( const vkUniform_t *uniform ) {
	const uint32_t offset = vk.cmd->uniform_read_offset = PAD( vk.cmd->vertex_buffer_offset, (VkDeviceSize)vk.uniform_alignment );
