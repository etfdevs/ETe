...
static void CMod_LoadLeafBrushes( const lump_t *l )
{
	...
	cm.leafbrushes = Hunk_Alloc( ( BOX_LEAF_BRUSHES + count ) * sizeof( *cm.leafbrushes ), h_current );
...
static void CMod_LoadEntityString( const lump_t *l, const char *name ) {
	...
	if (h && entFileLen > 0)
	{
		cm.entityString = (char *)Hunk_Alloc(entFileLen + 1, h_current );
	...
	cm.entityString = Hunk_Alloc( l->filelen, h_current );
...
static void CMod_LoadPatches( const lump_t *surfs, const lump_t *verts ) {
	...
	for ( i = 0 ; i < count ; i++, in++ ) {
		...
		if ( (uint64_t)width * height > MAX_PATCH_VERTS ) {
			Com_Error( ERR_DROP, "%s: MAX_PATCH_VERTS (%d)", __func__, MAX_PATCH_VERTS );
		}
