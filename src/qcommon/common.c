...
=============
*/
static int Q_PRINTF_FUNC(2, 0) QDECL Com_VPrintf( qboolean devPrint, const char *fmt, va_list argptr ) {
...
================
*/
void Q_PRINTF_FUNC(1, 2) QDECL Com_DPrintf( const char *fmt, ...) {
	va_list		argptr;
...
void Q_NO_RETURN Q_PRINTF_FUNC(2, 3) QDECL Com_Error( errorParm_t code, const char *fmt, ... ) {
	...
	} else if ( code == ERR_DROP || code == ERR_AUTOUPDATE ) {
		Com_Printf( "********************\nERROR: %s\n********************\n",
			com_errorMessage );
		VM_Forced_Unload_Start();
	...
	if ( !com_gameRestarting && com_fullyInitialized )
	{
		...
		if ( currentGameMod == GAMEMOD_UNKNOWN ) {
			...
			Com_Printf( S_COLOR_YELLOW "The ET:Legacy mod is considered experimental with this engine\n"
									"Not all ET:Legacy features will be functional\n" );
		}
	...
	if ( vendor ) {
		if ( cpuid_level_ex >= 0x80000004 ) {
			...
			if (print_flags) {
				...
				if (print_flags & CPU_SSE42)
					strcat(vendor, " SSE4.2");
			}
		}
	}
}
...
static void Com_WriteConfig_f( void ) {
	...
	if ( !FS_FilenameCompare( filename, "default.cfg" ) ) {
		Com_Printf( "%s: Not allowed to manipulate 'default.cfg' due to reserved name.\n", __func__ );
		return;
	}
