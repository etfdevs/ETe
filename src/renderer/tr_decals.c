                     ...
                     int fadeTime)
{
	...
	{
		ri.Printf( PRINT_WARNING, "WARNING: RE_ProjectDecal() Invalid number of decal points (%d)\n", numPoints);
		return;
	}
	...
	if (lifeTime == 0)
	{
		ri.Printf( PRINT_DEVELOPER, "WARNING: RE_ProjectDecal() lifeTime == 0\n"); // modders should have a look at this - vanilla does these calls
		return;
	}
	if (projection[3] <= 0.0f)
	{
		ri.Printf( PRINT_WARNING, "WARNING: RE_ProjectDecal() projection[3] <= 0.0f\n");
		return;
	}
	...
	else
	{
		...
		{
			ri.Printf( PRINT_WARNING, "WARNING: RE_ProjectDecal() MakeTextureMatrix returns NULL\n");
			return;
		}
	}
	...
	{
		ri.Printf( PRINT_DEVELOPER, "WARNING: RE_ProjectDecal() PlaneFromPoints is NULL\n"); // occurs on UJE_fueldump
		return;
	}
	...
	for (i = 0; i < numPoints; i++)
	{
		...
		{
			ri.Printf( PRINT_DEVELOPER, "WARNING: RE_ProjectDecal() a side plane is NULL\n"); // occurs on map venice
			return;
		}
	}
