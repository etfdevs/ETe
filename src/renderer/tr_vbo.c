...
static const char *BuildVP( int multitexture, int fogmode, int texgen )
{
	...
	switch ( fogmode ) {
		...
		case VP_FOG_NONE:
			break;
		case VP_FOG_EYE_IN:
			strcat( buf, fogVPCode ); break;
		case VP_FOG_EYE_OUT:
			strcat( buf, fogLevelVPCode ); break;
	}
...
void R_BuildWorldVBO( msurface_t *surf, int surfCount )
{
	...
	for ( i = 0, sf = surf; i < surfCount; i++, sf++ ) {
		...
		foliage = (srfFoliage_t *) sf->data;
		if ( foliage->surfaceType == SF_FOLIAGE && isStaticShader( sf->shader ) ) {
			foliage->vboItemIndex = ++numStaticSurfaces;
			numStaticVertexes += (foliage->numVerts * foliage->numInstances);
			numStaticIndexes += (foliage->numIndexes * foliage->numInstances);

			vbo_size += (foliage->numVerts * foliage->numInstances) * (sf->shader->svarsSize + sizeof( tess.xyz[0] ) + sizeof( tess.normal[0] ) );
			sf->shader->numVertexes += (foliage->numVerts * foliage->numInstances);
			sf->shader->numIndexes += (foliage->numIndexes * foliage->numInstances);
			continue;
		}
#ifdef USE_VBO_GRID
	...
	for ( i = 0, n = 0, sf = surf; i < surfCount; i++, sf++ ) {
		...
		foliage = (srfFoliage_t *) sf->data;
		if ( foliage->surfaceType == SF_FOLIAGE && foliage->vboItemIndex ) {
			surfList[ n++ ] = sf;
			continue;
		}
#ifdef USE_VBO_GRID
...
__fail:
	...
	for ( i = 0, sf = surf; i < surfCount; i++, sf++ ) {
		...
		foliage = (srfFoliage_t *) sf->data;
		if ( foliage->surfaceType == SF_FOLIAGE ) {
			foliage->vboItemIndex = 0;
			continue;
		}
#ifdef USE_VBO_GRID
		grid = (srfGridMesh_t *) sf->data;
...
static const fogProgramParms_t *VBO_SetupFog( int VPindex, int FPindex, GLuint *pvp, GLuint *pfp )
{
	...
	if ( !fparm->level ) 
	{
		...
		qglProgramLocalParameter4fARB( GL_VERTEX_PROGRAM_ARB, 4, fparm->eyeT, fparm->eyeInside, 0.0f, 0.0f );
	}
